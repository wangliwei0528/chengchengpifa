  ...
  <div class="user">
    <div class="top">
      <el-breadcrumb separator-class="el-icon-arrow-right">
        ...
        <el-breadcrumb-item>产品单位</el-breadcrumb-item>
      </el-breadcrumb>
      <div class="menu_input" style="margin-top: 30px">
        <div>
          <span style="font-size: 16px;margin-right: 10px">产品单位名称</span>
          <span>
        <el-input
          placeholder="搜索产品单位名称"
...
<script>
  ...
  export default {
    ...
    methods: {
      getTableData() {
        axios.get("/api/admin/category_unit_index",
          {
            headers: {
      ...
      editClick(index, row) {
        this.id = row.id
        this.mask = true;
        axios.get("api/admin/category_unit_edit_show/" + row.id,
          {
            headers: {
              'Authorization': 'Bearer ' + this.token,
              'Accept': 'application/vnd.lingmo.v1+json'
          ...
          .then(res => {
            ...
            let {title, sort, status,} = this.industry
            this.ruleForm = {
              inputName: title,
              inputSort: sort,
              inputStatus: `${status}`
      ...
      submission() {
        if (this.id) {
          ...
          axios.post("api/admin/category_unit_edit",
            data,
            {
              headers: {
                'Authorization': 'Bearer ' + this.token,
          ...
          axios.post("api/admin/category_unit_create",
            data,
            {
              headers: {
                'Authorization': 'Bearer ' + this.token,
      ...
      deleteClick(index, row) {
        axios.get("api/admin/category_unit_del/" + row.id,
          {
            headers: {
              'Authorization': 'Bearer ' + this.token,
              'Accept': 'application/vnd.lingmo.v1+json'
