  ...
  <div>
    <div class="editrole_center">
      <el-form ref="ruleForm" :model="ruleForm" :rules="rules" label-width="100px">
        ...
        <el-form-item label="产品规格 " prop="checkList" v-show="ruleForm.value1">
          <!--{{specs}} {{ruleForm.checkList}}-->
          <el-checkbox-group v-model="ruleForm.checkList" @change="specsChangeHandler">
            <el-checkbox v-for="val in specs" :label="val.id" :key="val.id">{{val.title}}</el-checkbox>
          </el-checkbox-group>
        </el-form-item>
        ...
        <el-form-item label="产品单位 " prop="value3" style="width: 300px">
          <el-select v-model="ruleForm.value3" style="width: 200px">
            <el-option v-for="item in units" :key="item.id" :label="item.title" :value="item.id"></el-option>
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="批发价格 " prop="inputTrade_price" style="width: 500px">
          <el-input
            ...
            placeholder="请输入出售价格"
            style="width: 400px"
            v-model="ruleForm.inputTrade_price"
          ></el-input>
        </el-form-item>
      ...
      <el-table border :data="tableData" v-show="specs" style="width:80%;margin-left: 100px">
            ...
            <div class="fileBox1" style='margin:0 auto;position:relative'>
              <span class="fileinput-button1">
                ...
                <input
                  ...
                  type="file"
                  name="img_path"
                  @change="getCoverFile($event, scope)"
                >
              </span>
            </div>
            <!--<input type="file"/>-->
        ...
        <el-table-column prop="sell" label="市场价格" align="center">
          <template slot-scope="scope">
            <el-input type="number" v-model="scope.row.sell" :min="0"/>
          </template>
        </el-table-column>
        <el-table-column prop="trade_price" label="批发价格" align="center">
          <template slot-scope="scope">
            <el-input type="number" v-model="scope.row.trade_price" :min="0"/>
          </template>
        </el-table-column>
        <el-table-column prop="quality" label="库存" align="center">
          <template slot-scope="scope">
            <el-input type="number" v-model="scope.row.quality" :min="0"/>
          </template>
        </el-table-column>
      </el-table>
    </div>
...
export default {
  ...
  methods: {
    ...
    getData() {
      axios
        .get("/api/admin/good_create_show", {
          headers: {
    ...
    uploadSuccessed(res, file, fileList) {
      this.ruleForm.banner.push(res.join());
    },
    uploadMore() {
    ...
    submission() {
      ...
      let data = {
        ...
        images: this.ruleForm.banner,
        content: this.ruleForm.content,
        skus: this.tableData,
        specId: this.specId
      };
      axios
        .post("api/admin/good_create", data, {
          headers: {
            Authorization: "Bearer " + this.token,
            Accept: "application/vnd.lingmo.v1+json"
          }
    ...
    specsChangeHandler(val) {
      ...
      axios
        ...
        .then(res => {
          this.specId = res.data.data.specId;
          let tmpArr = [];
          // console.log(vm.combinatePart(res.data.data))
          tmpArr = vm
            .combinatePart(res.data.data.data)
            .map(item => item.join(","));
          // 绑定表格数据
