  ...
  <div class="user">
    <div class="top">
      <el-breadcrumb separator-class="el-icon-arrow-right">
        ...
        <el-breadcrumb-item>产品品牌</el-breadcrumb-item>
      </el-breadcrumb>
      <div class="menu_input" style="margin-top: 30px">
        <div>
          <span style="font-size: 16px;margin-right: 10px">产品品牌名称</span>
          <span>
        <el-input
          placeholder="搜索产品品牌名称"
    ...
    <div class="editrole" v-if="mask">
      <div class="editrole_top">
        <div>新建(修改)产品品牌</div>
        <div @click="cancel"><button>×</button></div>
      </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      getTableData() {
        axios.get("/api/admin/category_brand_index",
          {
            headers: {
      ...
      editClick(index, row) {
        this.id = row.id
        this.mask = true;
        axios.get("api/admin/category_brand_edit_show/" + row.id,
          {
            headers: {
              'Authorization': 'Bearer ' + this.token,
              'Accept': 'application/vnd.lingmo.v1+json'
      ...
      submission() {
        if (this.id) {
          ...
          axios.post("api/admin/category_brand_create",
            data,
            {
              headers: {
                'Authorization': 'Bearer ' + this.token,
      ...
      deleteClick(index, row) {
        axios.get("api/admin/category_brand_del/" + row.id,
          {
            headers: {
              'Authorization': 'Bearer ' + this.token,
              'Accept': 'application/vnd.lingmo.v1+json'
